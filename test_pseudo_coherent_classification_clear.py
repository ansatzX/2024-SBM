...
def test_pseudo_coherent_classification():
    ...
    print(f"峰位置: {max_indices}")
    print(f"谷位置: {min_indices}")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("test_pseudo_coherent_trajectory_clear.png")
    print("轨迹图像已保存到 test_pseudo_coherent_trajectory_clear.png")
...
def test_pseudo_coherent_with_single_valley():
    ...
    t = np.arange(100)
    # 创建一个只有一个谷的轨迹
    trajectory = np.where(t < 30, 0.6 - 0.02 * t, 0.4 + 0.005 * (t - 30))
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("test_single_valley_trajectory_clear.png")
    print("轨迹图像已保存到 test_single_valley_trajectory_clear.png")
...
def test_pseudo_coherent_with_single_peak_and_valley():
    ...
    t = np.arange(100)
    # 创建一个有一个峰和一个谷的轨迹
    trajectory = np.where(t < 25, 0.5 + 0.01 * t,
                         np.where(t < 75, 0.75 - 0.005 * (t - 25), 0.5 + 0.002 * (t - 75)))
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("test_peak_valley_trajectory_clear.png")
    print("轨迹图像已保存到 test_peak_valley_trajectory_clear.png")
